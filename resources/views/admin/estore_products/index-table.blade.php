@if($list_items)
    @foreach($list_items as $list_item)
        <tr>
                            ...
                            @if($imagePath)
                                @php
                                    ...
                                    if (is_string($imagePath)) {
                                        ...
                                        // Multi-size object or array - extract URL
                                        $imageUrl = $imagePath['thumb'] ?? $imagePath['original'] ?? $imagePath['url'] ?? (is_array($imagePath) ? reset($imagePath) : $imagePath);
                                    } else {
                                        $imageUrl = $imagePath;
                                    }
                                @endphp
                                <div class="position-absolute" 
                                     ...
                                     onclick="viewEstoreProductImages({{ $list_item->id }}, {{ $index }})">
                                    <img src="{{ $imageUrl }}" 
            ...
            <td>
                <small>{{ $list_item->category->title }}</small>
            </td>
            <td>
                <small>{{ $list_item->price }}</small>
            </td>
            <td>
                <small>{{ $list_item->mrp }}</small>
            </td>
            <td>
                <small>{{ $list_item->stock }}</small>
            </td>
            <td><small>{{ $list_item->updated_at->format('d-m-Y, g:i A') }}</small></td>
            <td>
                @include('admin.crud.action-dropdown', [
                    'cloneUrl'    => url('admin/estore-products/'.$list_item->id.'/clone'),
                    'cloneTitle'  => 'Clone Estore Product',
                    'editUrl'     => url('admin/estore-products/'.$list_item->id.'/edit'),
                    'editTitle'   => 'Update Estore Product',
                    'deleteUrl'   => route('admin.estore-products.destroy', $list_item->id),
                    'redirectUrl' => route('admin.estore-products.index')
                ])
            </td>
        </tr>
    @endforeach
...
@push('scripts')
<script>
function viewEstoreProductImages(productId, startIndex) {
    // You can implement a lightbox/modal here to view all images
    console.log('View estore product images:', productId, 'Starting from index:', startIndex);
}
</script>
@endpush