...
class NurseService extends BaseService
{
    ...
    protected int $roleId = Role::NURSE->value;
    /**
    ...
    {
        $query = $this->model->where('role_id', $this->roleId)->with('userMeta');
    ...
    protected function mapUserMeta($user)
    {
        if (!$user->relationLoaded('userMeta')) {
            return $user;
        }

        $user->qualification = $user->userMeta
            ->firstWhere('meta_key', 'qualification')?->meta_value;

        $user->specialization = $user->userMeta
            ->firstWhere('meta_key', 'specialization')?->meta_value;

        $user->blood_group = $user->userMeta
            ->firstWhere('meta_key', 'blood_group')?->meta_value;

        $user->date_of_birth = $user->userMeta
            ->firstWhere('meta_key', 'date_of_birth')?->meta_value;

        $user->joining_date = $user->userMeta
            ->firstWhere('meta_key', 'joining_date')?->meta_value;
    ...
    {
        return $this->model->where('role_id', $this->roleId)->with('userMeta')->sorted()->get();
        foreach ($users as $user) {
            $user->qualification = $user->meta
                ->firstWhere('meta_key', 'qualification')?->meta_value;
    
            $user->specialization = $user->meta
                ->firstWhere('meta_key', 'specialization')?->meta_value;
    
            $user->blood_group = $user->meta
                ->firstWhere('meta_key', 'blood_group')?->meta_value;
    
            $user->date_of_birth = $user->meta
                ->firstWhere('meta_key', 'date_of_birth')?->meta_value;
    
            $user->date_of_joining = $user->meta
                ->firstWhere('meta_key', 'date_of_joining')?->meta_value;
        }
